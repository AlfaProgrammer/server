...
export class SaleController{
    ...
    async getAllSales(@Query() query){ //boolean string
         ...
         * second - Query parameter is set incorrec, so i must notify this strange behaviour to the user (front-end dev)
        ...
        if(Object.keys(query).length > 0){
            try {    
                if(query.isEstimate){ //if isEstimate key exist on query object, I have to parse the JSON, and pass the value to the service function
                    const isEstimateParsed = JSON.parse(query.isEstimate); //PARSE METHOD WILL THROW AN ERROR itself IF SOMETHING GOES WRONG DURING PARSING OPERATION
